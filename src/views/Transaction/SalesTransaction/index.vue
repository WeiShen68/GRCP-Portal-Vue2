<template>
    <div class="pa-sm-2">
        <v-card class="px-sm-3 py-2 transparent" outlined>
            <v-row align="center">
                <v-col cols="12" sm="8">
                    <div>
                        <span class="titles">Sales Transaction</span>

                        <v-btn class="ml-4" @click="openFilterModal">
                            <v-icon>mdi-magnify</v-icon>
                        </v-btn>
                        <filterMenu v-if="filterDialog" :stored_filter_data="passedData" @filterInput="filterValues"
                            :showModal2="filterDialog" @close="closeDialog">
                        </filterMenu>
                    </div>
                </v-col>
            </v-row>
        ...
        <v-card class="mt-3">
            <div class="d-flex align-center pt-4 pl-1">
                <v-row>
                    <v-col class="justify-start">
                        <span class="ml-3 table_label"> Listing - Sales Transactions
                            <span v-if="this.passedData.startDate && this.passedData.endDate"> - {{
                                this.passedData.startDate
                            }} to {{ this.date_test }} </span>
                            <span v-else-if="this.passedData.startDate && this.passedData.endDate == ''"> - From {{
                                this.passedData.startDate
                            }}</span>
                            <span v-else-if="this.passedData.startDate == '' && this.passedData.endDate"> - Till {{
                                this.date_test
                            }}</span>
                        </span>
                    </v-col>
            ...
            <div class="ma-4">
                ...
                <v-data-table :headers="headers" :items="sales" :search="search" :page.sync="pagination.page" disable-sort
                    :footer-props="{
                    ...
                    loading-text="Loading... Please wait" @click:row="openDialog">
...
export default {
    ...
    methods: {
        ...
        getTotalRec() {
            ...
            return transactionApi.salesTransactionList(params).then(res => {
        ...
        getList() {
            ...
            return transactionApi.salesTransactionList(params).then(res => {
                ...
                if (this.initialLoad) {
                    ...
                    this.initialRecObj.data = this.sales;
